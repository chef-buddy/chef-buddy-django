...
def user_to_recipe_counter(recipe_id_fc_list, user_fc_data):
    """Takes in user's flavor compounds and recipe flavor compounds to produce a dict of how many times the flavor
    compounds of the user appear in each recipe for the user. Each time a flavor compound appears, the score associated
    with the user's fc will be added to the recipe"""

    match_dict = {recipe_id:0 for recipe_id, fc_id in recipe_id_fc_list}
    recipe_fc_count = match_dict.copy()
